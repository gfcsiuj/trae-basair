...
const AIAssistant: React.FC = () => {
    ...
    // تم تحديث الموقع ليكون أعلى من شريط التبويبات العائم الجديد
    const fabBottomPosition = isAudioPanelOpen
        ? 'calc(15rem + env(safe-area-inset-bottom, 0rem))' // في حالة فتح مشغل الصوت
        : 'calc(7rem + env(safe-area-inset-bottom, 0rem))'; // في الوضع العادي
    ...
    return (
        <>
            <button 
                ...
                className={`ai-fab fixed left-5 w-19 h-19 bg-gradient-to-br from-primary to-primary-light text-white rounded-full flex items-center justify-center shadow-lg z-30 transition-all duration-300 hover:scale-110 ${state.isUIVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
